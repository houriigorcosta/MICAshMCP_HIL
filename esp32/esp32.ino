...
float out_maxOUTA = 25;// valor maximo da escala da saída A do processo simulado. Deve ser igual ao minimo_saida do simulink.

float out_minOUTB = 0; // valor minimo da escala da saída B do processo simulado. Deve ser igual ao maximo_saida do simulink.
float out_maxOUTB = 10;// valor maximo da escala da saída B do processo simulado. Deve ser igual ao minimo_saida do simulink.
...
void serialEvent() {
  if (MySerial.available()> 0) { // Verifica se tem algo a ser lido na porta serial
    ...
    if(inChar == 66){// Verifica se a saida a qual a parte da msg atual corresponde e a saída B e seta a flag
      VarIndex = true;
    }
    if (VarIndex == false){ // decodifica saida A
      if (stringRxStartA == true){ // Verifica se a string ja começou
      ...
      if (inChar == 32){ // verifica se a nova string começou. Detecta o espaço e seta a flag.
        stringRxStartA = true;
        incomingValueA = 0;
      }
    }
...
float Map (int x, int in_min, int in_max, int out_min, int out_max){ // mapeia um número de um range(escala) a outro.
  return ((float)x - in_min)*(out_max - out_min)/(in_max - in_min) + out_min;
}

void leituraEntrada(){ // Le a saída do plc, que é a entrada do processo simulado.
  yk = analogRead(AN_Pot1); // Le a entrada ADC
  yf = yf + alfa_f*(yk-yf); // filtro digital 1a. ordem sem atraso de amostragem

  if (mCtr % m ==0){ // amostra a variável filtrada decimada por m.
    ym = yf;
  }

  // Codificação para o formato necessário para envio ao simulink
...
void setup() {
  Serial.begin(9600); // configura comunicação serial com o monitor
  MySerial.begin(9600, SERIAL_8N1, 16, 17); // configura o canal serial utilizado para a comunicacao com Raspberry pi
